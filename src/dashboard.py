...
Covers all CSVs and TXTs generated by the C++.
"""
...
tab_corr, tab_metr, tab_geom, tab_ks, tab_delta, tab_gcluster, tab_susp, tab_heur, tab_fk, tab_num, tab_uni, tab_struct, tab_master, tab_sec = tabs

# ... (código existente omitido por brevidade)

# ── Tab: Secrets & Matrix ───────────────────────────────────────────────────
with tab_sec:
    st.header("🗝️ Deep Secrets & Hidden Streams")
    
    cun_path = os.path.join(OUTPUT_DIR, "cuneiform_stream.csv")
    if os.path.exists(cun_path):
        df_cun = pd.read_csv(cun_path)
        st.subheader("Cuneiform Base60 Stream")
        fig_cun = px.line(df_cun, x="Index", y="Decimal", title="Numerical mapping of Cuneiform data")
        st.plotly_chart(fig_cun, use_container_width=True)
        
        st.subheader("Rune Distribution in Stream (Val mod 29)")
        fig_dist = px.histogram(df_cun, x="RuneIndex", nbins=29, color_discrete_sequence=["#2ca02c"])
        st.plotly_chart(fig_dist, use_container_width=True)
        
    st.divider()
    st.subheader("DEEP_HASH Analysis")
    hash_img = os.path.join(OUTPUT_DIR, "deep_hash_plot.png")
    if os.path.exists(hash_img):
        st.image(hash_img, caption="Distribution of hexadecimal nibbles in the Deep Hash")
    
    st.divider()
    st.subheader("Magic Squares / Secret Matrices")
    import glob
    mat_imgs = glob.glob(os.path.join(OUTPUT_DIR, "secret_matrix_*.png"))
    if mat_imgs:
        cols = st.columns(len(mat_imgs))
        for i, img in enumerate(mat_imgs):
            cols[i].image(img, use_container_width=True)
            
    mp_hits = os.path.join(OUTPUT_DIR, "missing_primes_hits.txt")
    if os.path.exists(mp_hits):
        st.subheader("Missing Primes Gematria Hits")
        with open(mp_hits, "r") as f:
            st.text(f.read())
...
with tab_metr:
    ...
    if not os.path.exists(feat_path):
        ...
        extra_cols = [c for c in ["ChiSquare", "BigramIoC", "GpsSumMean", "TopDelta", "LRS", "LZ", "NumInterrupts"]
        ...
        with col2:
            ...
            if os.path.exists(feat_path_geom):
                ...
                if "Page" in df_feat_geom.columns and "TopDelta" in df_feat_geom.columns:
                    ...
                    st.info("page_features.csv not found or 'TopDelta' column missing. Run Options 1 or 9 in the C++ tool.")
...
with tab_ks:
    ...
    if not os.path.exists(ks_path):
        ...
        # Identificar páginas marcadas como UNRESOLVED para exibir no selectbox
        unresolved_set = set(df_ks[df_ks["KeyValue"] == "UNRESOLVED"]["Page"].unique())

        pages_ks = df_ks["Page"].unique().tolist()

        def format_ks_label(p):
            return f"⚠️ {p} (Unresolved)" if p in unresolved_set else f"✅ {p}"

        page_sel = st.selectbox("Select a page for stream analysis", pages_ks, format_func=format_ks_label)
        
        if page_sel in unresolved_set:
            st.warning(f"**Note:** {page_sel} is currently marked as UNRESOLVED. The values below represent the current modular difference between ciphertext and the raw input (or partial solution).")

        # Filtrar o marcador 'UNRESOLVED' (Posição -1) para não quebrar os gráficos
        df_pg = df_ks[(df_ks["Page"] == page_sel) & (df_ks["Position"] != -1)].copy()
        df_pg["KeyValue"] = pd.to_numeric(df_pg["KeyValue"])
        ...
        if os.path.exists(fft_img):
            st.image(fft_img, caption="FFT + Autocorrelation (subset of solved pages)", width="stretch")
...
with tab_fk:
        ...
        if os.path.exists(friedman_path):
            ...
            if {"Page", "KeyLen", "PeriodicIoC"}.issubset(df_fr.columns):
                ...
                fig_fr = px.imshow(
                    ...
                    title="Friedman Heatmap: Periodic IoC by KeyLen",
                )
        ...
        if os.path.exists(kasiski_path):
            ...
            if gcds:
                ...
                fig_gcd = px.bar(
                    ...
                    title="GCD Distribution (candidate key lengths)",
            ...
            st.warning("kasiski_results.txt not found. Run Option 15 in C++.")
    ...
    if os.path.exists(cluster_path):
        ...
        fig_cl = px.imshow(
            ...
            title="Mutual IoC between Cluster Pages",
        )
...
# ── Tab: Numerology (Gematria) ───────────────────────────────────────────
with tab_num:
    st.header("🔮 Gematria & Numerological Analysis")
    gem_csv = os.path.join(OUTPUT_DIR, "gematria_sums.csv")
    
    if os.path.exists(gem_csv):
        df_gem = load_csv(gem_csv)
        
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("Digital Root Distribution")
            fig_dr = px.histogram(df_gem, x="DigitalRoot", nbins=9, color_discrete_sequence=["#AB63FA"])
            fig_dr.update_xaxes(tickmode='linear', dtick=1)
            st.plotly_chart(fig_dr, use_container_width=True)
            
        with col2:
            st.subheader("Prime/Fibonacci Saliency")
            # Filtrar apenas o que é especial
            special = df_gem[(df_gem["IsPrime"] == 1) | (df_gem["IsFibonacci"] == 1)]
            st.metric("Total Special Words", len(special), f"{len(special)/len(df_gem)*100:.1f}% of text")
            
            fig_pie = px.pie(values=[df_gem["IsPrime"].sum(), df_gem["IsFibonacci"].sum(), df_gem["IsSquare"].sum()], 
                             names=["Primes", "Fibonacci", "Squares"], title="Property Overlap")
            st.plotly_chart(fig_pie, use_container_width=True)

        st.subheader("Deep Dive: Word Sums per Page")
        sel_p = st.selectbox("Select Page", df_gem["Page"].unique())
        df_p = df_gem[df_gem["Page"] == sel_p]
        
        fig_sums = px.scatter(df_p, x="WordIndex", y="Sum", color="DigitalRoot", 
                              size="Sum", hover_data=["IsPrime", "IsFibonacci"],
                              title=f"Word Sum Sequence: {sel_p}")
        st.plotly_chart(fig_sums, use_container_width=True)
    else:
        st.warning("gematria_sums.csv not found. Run Option 24 in C++.")
...
with tab_uni:
    ...
    if not os.path.exists(uni_path):
        ...
        imgs_uni = {
            "Distribution (bars)": uni_path.replace(".csv", "_dist.png"),
...
# ── Tab: Vocabulary & Structure ─────────────────────────────────────────────
with tab_struct:
    st.header("🏗️ Textual Structure & Vocabulary Overlap")
    
    col1, col2 = st.columns(2)
    
    vocab_path = os.path.join(OUTPUT_DIR, "vocabulary_overlap.csv")
    if os.path.exists(vocab_path):
        with col1:
            st.subheader("Vocabulary Jaccard Heatmap")
            df_vocab = load_csv_index(vocab_path)
            fig_v = px.imshow(df_vocab, color_continuous_scale="Viridis", title="Shared Runic Word Similarity")
            st.plotly_chart(fig_v, use_container_width=True)
            st.info("High overlap suggests shared plaintext or repeated cipher blocks.")
            
    struct_path = os.path.join(OUTPUT_DIR, "structural_segments.csv")
    if os.path.exists(struct_path):
        df_struct = load_csv(struct_path)
        with col2:
            st.subheader("Segment Length Distribution")
            s_type = st.radio("Segment Type", ["Word", "Line", "Clause"], horizontal=True)
            df_filt = df_struct[df_struct["Type"] == s_type]
            fig_s = px.violin(df_filt, x="Length", box=True, points="all", title=f"Distribution of {s_type} Lengths")
            st.plotly_chart(fig_s, use_container_width=True)
            
        st.subheader("Page Structural Comparison")
        # Mostrar média de tamanho de palavra por página
        stats = df_struct[df_struct["Type"] == "Word"].groupby("Page")["Length"].mean().reset_index()
        stats.columns = ["Page", "Avg Word Length"]
        fig_stats = px.bar(stats.sort_values("Avg Word Length"), x="Page", y="Avg Word Length", color="Avg Word Length")
        st.plotly_chart(fig_stats, use_container_width=True)
    else:
        st.warning("Run options 27 and 28 in C++ first.")

# ── Tab 9: Master Summary ──────────────────────────────────────────────────────
with tab_master:
