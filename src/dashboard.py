...
with tab_geom:
    ...
    if not os.path.exists(delta_path):
        ...
        with col1:
            ...
            fig_top = px.bar(top_d, x="Delta", y="Contagem", title="Top 15 Deltas")
        ...
        if os.path.exists(hist_img):
            st.image(hist_img, caption="Full histogram generated by visualize_correlations.py",
                     use_container_width=True)
...
with tab_ks:
    ...
    if not os.path.exists(ks_path):
        ...
        if os.path.exists(fft_img):
            st.image(fft_img, caption="FFT + Autocorrelation (all resolved pages)",
                     use_container_width=True)
...
with tab_heur:
    ...
    if not os.path.exists(heur_path):
        ...
        st.dataframe(df_heur, use_container_width=True)
...
with tab_delta:
    ...
    if os.path.exists(delta_csv) and os.path.exists(autocorr_csv):
        ...
        with col1:
            ...
            st.plotly_chart(fig_ds_hist, use_container_width=True)
        ...
        st.plotly_chart(fig_ds_seq, use_container_width=True)
    else:
